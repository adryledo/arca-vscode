# ARCA v0.0.1 Walkthrough

We have successfully built the foundation (Phase 1) and the maintainer workflow (Phase 2) of the **Protocol of Asset Resolution for Coding Agents (ARCA)**.
...
- **Dynamic Caching**: Assets are cached in `~/.arca-cache/` with LF-normalized SHA-256 validation.
- **Deterministic Locking**: Produced `.arca-assets.lock` to ensure permanent immutability, even if the source's `main` branch changes.
...
- **Integrity & Pinnning**:
    ...
    - **`ARCA: Update Asset`**: A manual command to explicitly move an asset to the latest commit on the source's `main` branch.
...
- **`ARCA: Publish Asset Version`**: A new command that enables maintainers to easily add new versions, auto-incrementing SemVer, and updating the manifest.
...
- **Fallback Support**: If VS Code auth is unavailable, falls back to environment variables (`GITHUB_TOKEN`, `AZURE_DEVOPS_PAT`, `ARCA_TOKEN`).
...
- **Vendor Neutrality**: The protocol uses vendor-neutral filenames like `.arca-assets.yaml` and `arca-manifest.yaml`.
